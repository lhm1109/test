...
st.session_state.setdefault("valid_data", False)
st.session_state.setdefault("valid_stld", False)
st.session_state.setdefault("valid_story", False)
...
# 빈 그래프용 더미 데이터
def plot_empty_graph(direction, graph_type, unit_force, unit_dist):
    dummy_df = pd.DataFrame({
        "STORY": [""],
        f"{graph_type} for {direction}": [0]
    })
    plot_graph(dummy_df, f"{graph_type} for {direction}", direction, unit_force, unit_dist)

# 빈 테이블용 더미 데이터프레임 생성
def get_empty_table(columns):
...
def fetch(base_url, mapi_key):
    ...
    try:
        ...
        if res_stld.status_code == 200:
            stld_data = res_stld.json().get("STLD", {}) or {}
        else:
            print(f"STLD fetch failed: {res_stld.status_code}")
    except Exception as e:
        print(f"STLD fetch exception: {e}")
    ...
    try:
        ...
        if res_story.status_code == 200:
            story_data = res_story.json().get("STORYPROP", {}).get("DATA", []) or []
        else:
            print(f"STORYPROP fetch failed: {res_story.status_code}")
    except Exception as e:
        print(f"STORYPROP fetch exception: {e}")
    ...
    try:
        ...
        if res_unit.status_code == 200:
            unit_data = res_unit.json().get("UNIT", {}).get("1", {}) or {}
        else:
            print(f"UNIT fetch failed: {res_unit.status_code}")
    except Exception as e:
        print(f"UNIT fetch exception: {e}")

    is_stld_valid = bool(stld_data)
    is_story_valid = bool(story_data)

    return stld_data, story_data, unit_data, is_stld_valid, is_story_valid
    ...
    if _base_url and _mapi_key and not st.session_state.api_connected:
        ...
        try:
            with st.spinner("Connecting to API..."):
                stld_data, story_data, unit_data, is_stld_valid, is_story_valid = fetch(base_url, _mapi_key)
                st.session_state.api_connected = True
                st.session_state.stld_data = stld_data
                st.session_state.story_data = story_data
                st.session_state.unit_data = unit_data
                st.session_state.valid_stld = is_stld_valid
                st.session_state.valid_story = is_story_valid
                st.session_state.valid_data = is_stld_valid and is_story_valid
                # 파싱 및 변환
                ...
                if is_story_valid:
                    ...
                    update_edc()
                if not is_stld_valid:
                    st.toast("❗ No Static Load Case data found. Please check your data.")
                if not is_story_valid:
                    st.toast("❗ No Story Property data found. Please check your data.")
                ...
                with refresh_button_col:
                    if st.button("🔄 Refresh", use_container_width=True, type="secondary"):
                        try:
                            ...
                            st.session_state.valid_stld = False
                            st.session_state.valid_story = False
                            st.toast(f"🔁 Refresh failed: {e}")

                with applied_button_col:
                    disabled_flag = not st.session_state.valid_data
                    apply_clicked = st.button("Apply Seismic Loads", use_container_width=True, type="primary", disabled=disabled_flag)
                                ...
                                body_x = {
                                    "Assign": {
                                        stld_key_map[load_case_x]: {
                                            ...
                                            "DESC": "AS 1170.4:2024 - X-Direction",
                                            "SCALE_FACTOR_X": 1,
                                            "SCALE_FACTOR_Y": 0,
                                            "SCALE_FACTOR_R": 0,
                                ...
                                body_y = {
                                    "Assign": {
                                        stld_key_map[load_case_y]: {
                                            "SEIS_LOAD_CODE": "USER TYPE",
                                            "DESC": "AS 1170.4:2024 - Y-Direction",
                                            "SCALE_FACTOR_X": 0,
                                            "SCALE_FACTOR_Y": 1,
                                            "SCALE_FACTOR_R": 0,
                                            "bACCIDENTAL_ECCEN": False,
        ...
        with c2:
            with st.container():
                ...
                if st.session_state.calculated:
                    df_x = st.session_state.df_x if not st.session_state.df_x.empty else pd.DataFrame()
                    df_y = st.session_state.df_y if not st.session_state.df_y.empty else pd.DataFrame()
                    ...
                    with tab1:
                        ...
                        with col2_1:
                            if not df_x.empty:
                                ...
                                plot_empty_graph("X", graph_type, unit_force, unit_dist)
                        with col2_2:
                            if not df_y.empty:
                                plot_graph(df_y, f"{graph_type} for Y", "Y", unit_force, unit_dist)
                            else:
                                plot_empty_graph("Y", graph_type, unit_force, unit_dist)
                    ...
                    with tab2:
                        ...
                        if table_dir == "X-Direction":
                            if not df_x.empty:
                                ...
                                display_df = display_df.rename(columns={col: label(col, unit_force, unit_dist) for col in display_df.columns})
                                display_df = display_df.round(2)
                            else:
                                display_df = get_empty_table(["STORY", "Story Force for X", "Story Shear for X", "Overturning Moment for X"])
                        else:
                            if not df_y.empty:
                                ...
                                display_df = get_empty_table(["STORY", "Story Force for Y", "Story Shear for Y", "Overturning Moment for Y"])

                        st.dataframe(display_df, height=600, use_container_width=True, hide_index=True)

                else:
                    st.warning("Please fill in story data to view results.")
