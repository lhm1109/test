...
# Set fatigue method
st.session_state.Fatigue_method = "Steel Girder(Direct Stress)"

# Get import dialogs
import_dialogs = SessionManager.initialize_import_dialogs() if 'SessionManager' in globals() else None
civil_stress_import = import_dialogs['steel_stress'] if import_dialogs else None
...
#피로검토계산 피로계산 결과계산
def fatigue_steel_girder_direct_calculate():
    """Calculate fatigue verification for steel girder"""
    # Get input values
    gamma_m = st.session_state.get('factor_rm', 1.35)
    delta_sigma_amm = st.session_state.get("delta_sigma_amm", 180)
    delta_sigma1 = st.session_state.get("delta_sigma1", 0)
    delta_sigma12 = st.session_state.get("delta_sigma12", 0)
    
    # Calculate design values
    delta_sigma_rsk = delta_sigma_amm / gamma_m
    
    # Calculate equivalent stress
    lambda_s = st.session_state.get('calc_lambda_s', 0.0)
    delta_sigma_equ = delta_sigma1 * lambda_s
    
    # Calculate verification ratio
    direct_stress_ratio = delta_sigma_equ / delta_sigma_rsk
    
    # Save results to session state
    st.session_state.delta_sigma1 = delta_sigma1
    st.session_state.calc_delta_sigma_rsk = delta_sigma_rsk
    st.session_state.calc_delta_sigma_equ = delta_sigma_equ
    st.session_state.calc_direct_stress_ratio = direct_stress_ratio
    st.session_state.calc_is_ok = direct_stress_ratio <= 1.0
    st.session_state.is_ok = "OK" if direct_stress_ratio <= 1.0 else "NG"
    
    # Update temp_result_df
    if 'temp_result_df' in st.session_state and not st.session_state.temp_result_df.empty:
        st.session_state.temp_result_df.at[0, 'delta_sigma_rsk'] = delta_sigma_rsk
        st.session_state.temp_result_df.at[0, 'delta_sigma_equ'] = delta_sigma_equ
        st.session_state.temp_result_df.at[0, 'direct_stress_ratio'] = direct_stress_ratio
        st.session_state.temp_result_df.at[0, 'is_ok'] = st.session_state.is_ok
    
    # Update multiple fields to temp_result_df
    update_temp_from_input_multi(['case_name', 'delta_sigma_amm', 'span_length', 'direct_stress_ratio',
                                 'delta_sigma1', 'delta_sigma12', 'Fatigue_method', 'is_ok', 'delta_sigma1'])
    
def fatigue_steel_girder_direct_ui_page():
    st.session_state.Fatigue_method = "Steel Girder(Direct Stress)"
    ...
    if is_edit_mode:
        st.markdown(f"<h5><b>[Edit] Steel Girder (Direct Stress) : {st.session_state.get('fatigue_case_name', '')}</b></h5>", unsafe_allow_html=True)
    else:
        st.markdown("<h5><b>Steel Girder (Direct Stress)</b></h5>", unsafe_allow_html=True)
    ...
    with tab1:
        with st.container(height=800, border=False):
            ...
            st.text_input("Case name", value=default_case_name, key='case_name', 
                        on_change=update_temp_from_input, args=('case_name', 'newcase'))
            
            # Check for duplicate case names (only for new cases)
            case_name = st.session_state.get('case_name', 'newcase')
            is_duplicate = False
            if not is_edit_mode:
            ...
            span_length = st.number_input(
                ...
                min_value=0.5,
                max_value=100.0,
                value=st.session_state.get('span_length', 35.000),
            ...
            with col1:
                detail_category = st.selectbox(
                    "Detail Category", 
                    [
                        "160", "140", "125", "112", "100", "90", "80", "71", "63", "56", 
                        "50", "45", "40", "36"
                    ], 
                    index=["160", "140", "125", "112", "100", "90", "80", "71", "63", "56", 
                        "50", "45", "40", "36"].index(str(st.session_state.get('detail_category', "160"))),
                    key="detail_category_widget", 
                    on_change=update_temp_from_input, 
                    args=("detail_category_widget", "detail_category"),
                    help="Select the detail category for fatigue strength calculation (based on EN 1993-1-9)"
                )

            with col2:
                # detail_category 값을 먼저 가져옴
                detail_cat = st.session_state.get('detail_category', "160")
                
                # detail_category 값을 float으로 변환하여 delta_sigma_amm의 기본값으로 사용
                delta_sigma_amm = st.number_input(
                    r"Nominal fatigue Stress Range, $\Delta\sigma_{amm}$ (MPa)", 
                    value=float(detail_cat),
                    key='delta_sigma_amm_widget', 
                    on_change=update_temp_from_input, 
                    args=('delta_sigma_amm_widget', 'delta_sigma_amm'),
                    disabled=True
                )
            ...
            st.markdown("<h6><b>Direct Stress Load</b></h6>", unsafe_allow_html=True)
            # Load import toggle
            st.toggle("Import From Midas NX",
                value=st.session_state.get('manual_input_direct', True),
            ...
            with st.container(border=True):
                ...
                with col1:
                    if st.button("Import", use_container_width=True, key="from_midas_nx_direct", 
                                disabled=not st.session_state.manual_input_direct):
                        civil_stress_import("Import Direct Stress")
                
                with col2:
                    if not st.session_state.manual_input_direct:
                    ...
                    st.number_input(
                        ...
                        value=float(st.session_state.get('delta_sigma_1', 192.0)),  # 초기값만 설정
                        step=1.0, 
                        key="widget_delta_sigma_1",  # 실제 값은 이 키로 관리됨
                        on_change=update_temp_from_input,
                ...
                with col3:
                    st.number_input(
                        ...
                        value=float(st.session_state.get('delta_sigma_12', st.session_state.get('temp_delta_sigma_12', 0))), 
                        step=1.0, 
        ...
        with col3:
            ...
            validation_errors1 = (
                is_duplicate or 
                st.session_state.get('delta_sigma1', 0) <= 0 or 
                st.session_state.get('delta_sigma12', 0) <= 0 or
                st.session_state.get('span_length', 0) <= 0 
            )
            
            if validation_errors1:
                if st.button("Next →", use_container_width=True, key="next_button_with_error"):
                    if is_duplicate:
                        st.toast("Case name already exists", icon="⚠️")
                    elif st.session_state.get('delta_sigma1', 0) <= 0:
                        st.toast("Fatigue load ($\Delta\sigma_1$) is required", icon="⚠️")
                    elif st.session_state.get('delta_sigma12', 0) <= 0:
                        st.toast("Fatigue load ($\Delta\sigma_{1+2}$) is required", icon="⚠️")
                    elif st.session_state.get('span_length', 0) <= 0:
                        st.toast("Span length ($L$) is required", icon="⚠️")
                    elif st.session_state.get('case_name', 0) == "":
                ...
                if st.button("Next →", use_container_width=True, type="primary", key="next_button_without_error"):
                    # Save all current values to temp_result_df
                    update_temp_from_input_multi(['delta_sigma1', 'delta_sigma12', 'span_length','manual_input_direct', 'detail_category'])
    ...
    with tab2:
        with st.container(height=800, border=False):
            ...
            if st.session_state.get('bridge_type') == "Railway":
                with col1:
                    def update_category_and_type():
                        ...
                        update_temp_from_input_multi(['traffic_category', 'traffic_type_steel'])
                    
                    options = ["Standard", "Multiple/Underground", "25t per axle"]
                    current_value = st.session_state.get('traffic_category', "Standard")
                    current_index = options.index(current_value) if current_value in options else 0
                    
                    traffic_category = st.selectbox(
                        "Traffic Category",
                        options,
                        index=current_index,  # value 대신 index 사용
                        key="traffic_category_widget",
                        on_change=update_category_and_type,
                    )
                    ...
                    #     r"Design life of structure (years), $t_{LD}$", 
                    #     min_value=50, max_value=120,
                    #     value=int(st.session_state.get('design_life', 50)), 
                    #     step=10, 
                    #     key="design_life_widget",
                    #     on_change=update_temp_from_input,
                    #     args=("design_life_widget", "design_life")
                    # )
                with col2:
            ...
            if st.button("Correction Factor Calculate", use_container_width=True, 
                ...
                try:
                    ...
                    # 피로 계산 수행
                    fatigue_steel_girder_direct_calculate()

                    st.toast("Fatigue factors calculated successfully!", icon="✅")
        ...
        # Lambda 값들이 변경될 때마다 lambda_s 자동 계산하는 함수
            ...
            with col1:
                ...
                st.number_input(
                    r"$\lambda_{1}$", 
                    min_value=0.0, 
                    value=float(st.session_state.get('calc_lambda1', 0.0)),
                    step=0.05, 
                ...
                st.number_input(
                    r"$\lambda_{3}$", 
                    min_value=0.0, 
                    value=float(st.session_state.get('calc_lambda3', 0.0)),
                    step=0.05, 
                ...
                st.number_input(
                    r"$\lambda_{s}$", 
                    min_value=0.0, 
                    value=float(st.session_state.get('calc_lambda_s', 0.0)),
                    step=0.05, 
            ...
            with col2:
                ...
                st.number_input(
                    r"$\lambda_{2}$", 
                    min_value=0.0, 
                    value=float(st.session_state.get('calc_lambda2', 0.0)),
                    step=0.05, 
            ...
            if st.session_state.get('correction_factor_auto_calculate', True) == True:
                ...
                if lambdaorigin > st.session_state.get('calc_lambda_max', 2.0):
                    ...
                    st.markdown("<small></small>", unsafe_allow_html=True)
            st.session_state['verification_options'] = ["Direct Stress"] 
        # Navigation buttons
        col1, col2, col3 = st.columns(3)
        with col1:
        ...
        with col3:
            ...
            def has_correction_validation_errors2():
                if st.session_state.get('bridge_type') == "Railway":
                    return (
                        st.session_state.get('annual_traffic', 0) <= 0 or
                        st.session_state.get('design_life', 0) <= 0 or
                        not st.session_state.get('traffic_category') or
                        not st.session_state.get('traffic_type_steel') or
                        not st.session_state.get('calc_lambda_s')
                    )
                else:
                    return (
                        st.session_state.get('nobs', 0) <= 0 or
                        st.session_state.get('qmk', 0) <= 0 or
                        not st.session_state.get('calc_lambda_s')
                    )
            ...
            if validation_errors2:
                if st.button("Next →", use_container_width=True, key="next_button_with_error2"):
                    update_temp_from_input_multi(['annual_traffic', 'design_life', 'nobs', 'qmk'])
                    if st.session_state.get('bridge_type') == "Railway":
                        if st.session_state.get('annual_traffic', 0) <= 0:
                            st.toast("Annual traffic volume is required", icon="⚠️")
                        elif not st.session_state.get('traffic_category'):
                            st.toast("Traffic category is required", icon="⚠️")
                        elif not st.session_state.get('traffic_type_steel'):
                            st.toast("Traffic type is required", icon="⚠️")
                        elif not st.session_state.get('calc_lambda_s'):
                            st.toast("Lambda s is required", icon="⚠️")
                    else:
                        if st.session_state.get('nobs', 0) <= 0:
                            st.toast("Number of lorries per year is required", icon="⚠️")
                        elif st.session_state.get('qmk', 0) <= 0:
                            st.toast("Average gross weight of lorries is required", icon="⚠️")
                        elif not st.session_state.get('calc_lambda_s'):
                            st.toast("Lambda s is required", icon="⚠️")
            else:
                if st.button("Calculate Fatigue Result  →", use_container_width=True, type="primary", key="next_button_without_error2"):
                    update_temp_from_input_multi(['pie_fat','annual_traffic', 'design_life', 'nobs', 'qmk', 'location', 'traffic_category', 'detail_category',
                                                  'traffic_type_steel', 'correction_factor_auto_calculate', 'calc_lambda1', 'calc_lambda2', 'calc_lambda3', 'calc_lambda4', 'calc_lambda_s', 'calc_lambda_max'])
    ...
    with tab3:
        ...
        if validation_errors2 or validation_errors1 or final_check():
            print(final_check())
            if st.button("Calculate Fatigue Result", key="calculate_fatigue_result", use_container_width=True):
                ...
                if not validation_errors2 and not validation_errors1:
                    if st.session_state.get('is_ok') == "N/A":
            ...
            with st.container(border=True, height=800):
                if ('calc_direct_stress_ratio' in st.session_state and "Direct Stress" in st.session_state.get('verification_options', [])):
                    if st.session_state.get('input_changed', False):
                        ...
                        if st.button("Fatigue Result Recalculate", key="calculate_fatigue_result2", use_container_width=True):
                            ...
                            st.rerun()
                st.markdown("<h6><b>Steel Girder Fatigue Result (Direct Stress)</b></h6>", unsafe_allow_html=True)
                
                # Direct stress check result display
                if 'calc_direct_stress_ratio' in st.session_state and "Direct Stress" in st.session_state.get('verification_options', []):
                    with st.container(border=True):
                        direct_stress_ratio = st.session_state.calc_direct_stress_ratio
                        delta_sigma_equ = st.session_state.get('calc_delta_sigma_equ', 0.0)
                        delta_sigma_rsk = st.session_state.get('calc_delta_sigma_rsk', 0.0)
                        is_ok = direct_stress_ratio <= 1.0
                        
                        col1, col2, col3, col4 = st.columns([10, 1, 9, 5], vertical_alignment="center")
                        with col1:
                            st.latex(r"\Delta\sigma_{equ} = " + f"{delta_sigma_equ:.3f}\t{{ MPa}}")
                        with col2:
                            st.latex(r"\leq" if is_ok else r">")
                        with col3:
                            st.latex(r"\Delta\sigma_{Rsk} = " + f"{delta_sigma_rsk:.3f}\t{{ MPa}}")
                        with col4:
                            if is_ok:
                ...
                st.markdown("<h6><b>Detail Calculation of Steel Fatigue Result (Direct Stress)</b></h6>", unsafe_allow_html=True)
                if st.session_state.correction_factor_auto_calculate ==False:
                    ...
                    if st.session_state.get('bridge_type') == "Railway":
                        ...
                        with st.container(border=True):        
                            st.latex(
                                rf"\lambda_{{2}} = f(N_{{obs}}, Q_{{mk}}) = f({nobs:.0f}, {qmk:.0f}\,\mathrm{{kN}}) = {lambda2:.3f}"
                            )
                ...
                if "Direct Stress" in st.session_state.get('verification_options', []):
                    delta_sigma1 = st.session_state.get('delta_sigma1', 0.0)
                    delta_sigma12 = st.session_state.get('delta_sigma12', 0.0)
                    lambda_s = st.session_state.get('calc_lambda_s', 0.0)
                    delta_sigma_equ = st.session_state.get('calc_delta_sigma_equ', 0.0)
                    st.markdown(r"Equivalent direct stress range: $\Delta\sigma_{equ} = \Delta\sigma_{1} \cdot \lambda_{s} = " + 
                            f"{delta_sigma1:.3f}" + r" \cdot " + f"{lambda_s:.3f}" + r" = " + f"{delta_sigma_equ:.3f}" + r"\text{ MPa}$", 
                            unsafe_allow_html=True)

                # Design strength section
                gamma_m = st.session_state.get('factor_rm', 1.35)

                # Direct stress strength
                if "Direct Stress" in st.session_state.get('verification_options', []):
                    delta_sigma_amm = st.session_state.get('delta_sigma_amm', 180.0)
                    delta_sigma_rsk = st.session_state.get('calc_delta_sigma_rsk', 0.0)
                    st.markdown(r"Design direct stress fatigue strength: $\Delta\sigma_{Rsk} = \frac{\Delta\sigma_{C}}{\gamma_{M}} = \frac{" + 
                            f"{delta_sigma_amm:.3f}" + "}{" + f"{gamma_m:.3f}" + "} = " + f"{delta_sigma_rsk:.3f}" + r"\text{ MPa}$", 
                            unsafe_allow_html=True)

                # Direct stress verification
                if "Direct Stress" in st.session_state.get('verification_options', []):
                    delta_sigma_equ = st.session_state.get('calc_delta_sigma_equ', 0.0)
                    delta_sigma_rsk = st.session_state.get('calc_delta_sigma_rsk', 0.0)
                    direct_stress_ratio = st.session_state.get('calc_direct_stress_ratio', 0.0)
                    ...
                    with st.container(border=True):
                        # 부등식 표시
                        st.latex(r"\Delta\sigma_{equ} \leq \Delta\sigma_{Rsk}")
                        
                        # 실제 값과 판정 표시
                        st.latex(f"{delta_sigma_equ:.3f}" + r" \text{ MPa} " + 
                                (r"\leq " if is_ok else r"> ") + 
                                f"{delta_sigma_rsk:.3f}" + r" \text{ MPa} " +
                                (r"\;\; \therefore \text{OK}" if is_ok else r"\;\; \therefore \text{NG}"))
                        
                        # 비율 표시
                        st.latex(r"\frac{\Delta\sigma_{equ}}{\Delta\sigma_{Rsk}} = " + 
                                f"{direct_stress_ratio:.3f} " +
        ...
        with col3:
            ...
            if st.button(button_text, key="save_fatigue_result", use_container_width=True, type="primary"):
                ...
                update_temp_from_input_multi(['pie_fat','annual_traffic', 'design_life', 'nobs', 'qmk', 'location', 'traffic_category', 'detail_category',
                                                'traffic_type_steel', 'correction_factor_auto_calculate', 'calc_lambda1', 'calc_lambda2', 'calc_lambda3', 'calc_lambda4', 'calc_lambda_s', 'calc_lambda_max'])
                if 'calc_direct_stress_ratio' not in st.session_state:
                    if not fatigue_steel_girder_direct_calculate():
                ...
                if 'temp_result_df' in st.session_state and not st.session_state.temp_result_df.empty:
                    ...
                    # 판정값 저장
                    st.session_state.temp_result_df.at[0, 'direct_stress_ratio'] = st.session_state.get('calc_direct_stress_ratio', 0)
                    st.session_state.temp_result_df.at[0, 'is_ok'] = st.session_state.is_ok
...
def calculate_lambda_coefficients():
    ...
    if st.session_state.get('bridge_type') == "Railway":
        ...
        span_length = st.session_state.get('span_length', 35.0)
        annual_traffic = st.session_state.get('annual_traffic', 25.0)
        design_life = st.session_state.get('nyear', 50)
        ...
        design_life = st.session_state.get('nyear',)
        location = st.session_state.get('location', "midspan")
...
자동 계산값  
- calc_delta_sigma_rsk  
- calc_delta_sigma_equ  
- calc_direct_stress_ratio  
- calc_is_ok  
- is_ok  
