...
class CombinedBaseSection(BaseSection):
    """
    DBUSER 단면 기본 클래스
    ...
    def __init__(self, section_data, unit_system):
        ...
        self.data_type = section_data.get("SECT_BEFORE",{}).get("DATATYPE",0)
    ...
    def calculate_vertices(self):
        ...
        if self.data_type not in (1, 2):
            self.vertices_i = {
                ...
                "error_code": "COMBINEDBASESECTION_CALCULATE_VERTICES"
            }
        ...
        if self.data_type == 1:
            ...
            vSize_i = self.section_data.get("SECT_BEFORE", {}).get("SECT_I", {}).get("vSIZE", [])
            vSize_j = self.section_data.get("SECT_BEFORE", {}).get("SECT_J", {}).get("vSIZE", [])

            if not vSize_i or not vSize_j:
                self.vertices_i = {
            ...
            dimensions = self._get_dimensions_from_vsize(vSize_i, vSize_j)
        ...
        self.vertices_i, self.reference_points_i, self.properties_control_i = self._generate_shape_vertices(dimensions)
