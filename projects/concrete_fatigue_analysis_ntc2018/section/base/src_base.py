...
class SRCBaseSection(BaseSection):
    """
    SRC 단면 기본 클래스
    ...
    def __init__(self, section_data, unit_system):
        ...
        self.data_type = section_data.get("SECT_BEFORE", {}).get("SRC_TYPE", 3)
    ...
    def calculate_vertices(self):
        ...
        if self.data_type == 1:
            ...
            steel_size = self.section_data.get("SECT_BEFORE", {}).get("SECT_I", {}).get("vSIZE", [])
            concrete_size = self.section_data.get("SECT_BEFORE", {}).get("SECT_J", {}).get("vSIZE", [])\
            ...
            options = {
                ...
                "matl_elast": self.section_data.get("SECT_BEFORE", {}).get("MATL_ELAST", 0),
                "matl_dens": self.section_data.get("SECT_BEFORE", {}).get("MATL_DENS", 0),
                "matl_pois_s": self.section_data.get("SECT_BEFORE", {}).get("MATL_POIS_S", 0),
                "matl_pois_c": self.section_data.get("SECT_BEFORE", {}).get("MATL_POIS_C", 0),
                "matl_stif_factor": self.section_data.get("SECT_BEFORE", {}).get("MATL_STIF_FACTOR", 0)
            }
            
            if not steel_size:
                self.vertices_i = {
                    "error": "vSize is not found",
                    "error_code": "SRCBASESECTION_CALCULATE_VERTICES"
                }
                return self.vertices_i, self.reference_points_i, self.properties_control_i
