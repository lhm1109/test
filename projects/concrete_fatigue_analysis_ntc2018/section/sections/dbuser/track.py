...
class TrackSection(DBUserValueBaseSection):
    """DBUSER: Track Section"""
    ...
    def _generate_shape_vertices(self, dimensions, options={}):
        ...
        # Inner
        arc_inner_left = ShapeGenerator.generate_arc_shape(R-t, 180, 360, (-(B-H)/2, -H/2))
        arc_inner_right = ShapeGenerator.generate_arc_shape(R-t, 0, 180, ((B-H)/2, -H/2))
        
        merged_inner = TransFormGeometry.merge_coordinates(arc_inner_left, arc_inner_right)
        reverse_inner = TransFormGeometry.reverse_coordinates_list(merged_inner)
        closed_inner = TransFormGeometry.close_polygon(reverse_inner)
